...
// Get the OpenGL version number
    ...
    void gltMakeTorus(ModelAsset& modelAsset, GLfloat majorRadius, GLfloat minorRadius, GLint numMajor, GLint numMinor)
    {
        ...
        double minorStep = 2.0f*M3D_PI / numMinor;
        int i, j;
        GLfloat datas[8 * 4 * numMinor * (numMinor + 1)];
        for (i=0; i<numMajor; ++i)
        {
            double a0 = i * majorStep;
            double a1 = a0 + majorStep;
            ...
            for (j=0; j<=numMinor; ++j)
            {
                ...
                // First point
                // texture
                datas[jindex + j * 8 * 4 + 3] = (float)(i)/(float)(numMajor);
                datas[jindex + j * 8 * 4 + 4] = (float)(j)/(float)(numMinor);
                // normal
                M3DVector3f normal = {x0*c, y0*c, z/minorRadius};
                m3dNormalizeVector3(normal);
                datas[jindex + j * 8 * 4 + 5] = normal[0];
                datas[jindex + j * 8 * 4 + 6] = normal[1];
                datas[jindex + j * 8 * 4 + 7] = normal[2];
                // vertex
                datas[jindex + j * 8 * 4 + 0] = x0 * r;
                datas[jindex + j * 8 * 4 + 1] = y0 * r;
                datas[jindex + j * 8 * 4 + 2] = z;

                // Second point
                // texture
                datas[jindex + j * 8 * 4 + 8 * 1 + 3] = (float)(i+1)/(float)(numMajor);
                datas[jindex + j * 8 * 4 + 8 * 1 + 4] = (float)(j)/(float)(numMinor);
                // normal
                normal = {x1*c, y1*c, z/minorRadius};
                m3dNormalizeVector3(normal);
                datas[jindex + j * 8 * 4 + 8 * 1 + 5] = normal[0];
                datas[jindex + j * 8 * 4 + 8 * 1 + 6] = normal[1];
                datas[jindex + j * 8 * 4 + 8 * 1 + 7] = normal[2];
                // vertex
                datas[jindex + j * 8 * 4 + 8 * 1 + 0] = x1*r;
                datas[jindex + j * 8 * 4 + 8 * 1 + 1] = y1*r;
                datas[jindex + j * 8 * 4 + 8 * 1 + 2] = z;
                ...
                // Third (based on first)
                // texture
                datas[jindex + j * 8 * 4 + 8 * 2 + 3] = (float)(i)/(float)(numMajor);
                datas[jindex + j * 8 * 4 + 8 * 2 + 4] = (float)(j + 1)/(float)(numMinor);
                // normal
                normal = {x0*c, y0*c, z/minorRadius};
                m3dNormalizeVector3(normal);
                datas[jindex + j * 8 * 4 + 8 * 2 + 5] = normal[0];
                datas[jindex + j * 8 * 4 + 8 * 2 + 6] = normal[1];
                datas[jindex + j * 8 * 4 + 8 * 2 + 7] = normal[2];
                // vertex
                datas[jindex + j * 8 * 4 + 8 * 2 + 0] = x0*r;
                datas[jindex + j * 8 * 4 + 8 * 2 + 1] = y0*r;
                datas[jindex + j * 8 * 4 + 8 * 2 + 2] = z;

                // Fourth (based on second)
                // texture
                datas[jindex + j * 8 * 4 + 8 * 3 + 3] = (float)(i+1)/(float)(numMajor);
                datas[jindex + j * 8 * 4 + 8 * 3 + 4] = (float)(j+1)/(float)(numMinor);
                // normal
                normal = {x1*c, y1*c, z/minorRadius};
                m3dNormalizeVector3(normal);
                datas[jindex + j * 8 * 4 + 8 * 3 + 5] = normal[0];
                datas[jindex + j * 8 * 4 + 8 * 3 + 6] = normal[1];
                datas[jindex + j * 8 * 4 + 8 * 3 + 7] = normal[2];
                // vertex
                datas[jindex + j * 8 * 4 + 8 * 3 + 0] = x1*r;
                datas[jindex + j * 8 * 4 + 8 * 3 + 1] = y1*r;
                datas[jindex + j * 8 * 4 + 8 * 3 + 2] = z;

            }
        }
        modelAsset.drawType = GL_TRIANGLES;
    ...
    void gltMakeCylinder(TriangleBatch& cylinderBatch, GLfloat baseRadius, GLfloat topRadius,
    ...
    void gltMakeCylinderAsset(ModelAsset& cylinderBatch, GLfloat baseRadius, GLfloat topRadius,
                         GLfloat fLength, GLint numSlices, GLint numStacks)
    {
        float fRadiusStep = (topRadius - baseRadius) / float(numStacks);
    ...
    void PfCOgl::gltLoadShaderSrc(const char *szShaderSrc, GLuint shader)
    {
    ...
    M3DMatrix44f PfCOgl::gltOrthoMatrix(GLfloat xMin, GLfloat xMax, GLfloat yMin, GLfloat yMax, GLfloat zMin, GLfloat zMax){
    ...
    bool PfCOgl::gltLoadShaderFile(const char *szFile, GLuint shader)
    {
    ...
    void gltGenerateOrtho2DMat(GLuint screenWidth, GLuint screenHeight,
                               M3DMatrix44f &orthoMatrix, Batch &screenQuad)
    {
        ...
        // set ortho matrix
        orthoMatrix[0] = {(float)(2 / (right - left)),0,0,0};
        
        orthoMatrix[1] = {0.0,(float)(2 / (top - bottom)),0,0};
        
        orthoMatrix[2] = {0.0,0,(float)(-2 / (1.0 - 0.0)),0};
        
        orthoMatrix[3] = {-1*(right + left) / (right - left),-1*(top + bottom) / (top - bottom),-1.0f,1.0};
