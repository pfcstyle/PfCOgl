...
static PfCOgl::Program* LoadShaders(const char* vertFilename, const char* fragFilename) {
    ...
    return PfCOgl::Program::getProgramByShadersWithAttr(shaders,2,GLT_ATTRIBUTE_VERTEX, "vVertex",GLT_ATTRIBUTE_TEXTURE0, "vTexCoord");
}
...
void loadAssetAndInstances() {
    ...
    logoAsset.shaders = LoadShaders("vp.glsl", "fp.glsl");
    //top left coord
    int logox = 500;
    int logoy = 155;
    int width = 300;
